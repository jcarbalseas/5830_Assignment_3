...
def is_ordered_block(w3, block_num):
	...
	transactions = block['transactions'] #added by myself
	ordered = False
	...
	for tx_hash in transactions:
		...
		if 'maxPriorityFeePerGas' in tx and 'maxFeePerGas' in tx:
			current_fee = min(tx['maxPriorityFeePerGas'], tx['maxFeePerGas'] - block['baseFeePerGas'])
		else:
...
def get_contract_values(contract, admin_address, owner_address):
	...
	has_role = contract.functions.hasRole(contract.DEFAULT_ADMIN_ROLE(), admin_address).call()  # Check the contract to see if the address "admin_address" has the role "default_admin_role"
